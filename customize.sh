...
if [ "`grep_prop dolby.mod $OPTIONALS`" != 1 ]; then
  ...
  if [ "$FILE" ]; then
    ...
    ui_print "  Optionals at the README."
  fi
  ui_print " "
  ...
  || [ -f $PRODUCT$FILE ]; then
    ...
    ui_print "  you need to use dolby.mod=1 to fix that."
    ui_print "  Please check Optionals at the README."
    ui_print " "
  fi
fi
...
rm -rf $MODPATH/system_support $MODPATH/unused
...
# conflict
if [ "`grep_prop dolby.mod $OPTIONALS`" == 1 ]; then
  NAMES="dolbyatmos MotoDolby DolbyAudio DolbyAtmosSP
         DolbyAtmos360 DolbyAtmosSpatialSound"
else
  NAMES="dolbyatmos MotoDolby DolbyAudio DolbyAtmosSP
         DolbyAtmos360 DolbyAtmosSpatialSound
         dsplus Dolby"
